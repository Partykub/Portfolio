        ...
        <Card className="col-span-1 md:col-span-3 bg-transparent border-none shadow-none p-0 overflow-visible">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {items.map((item, idx) => (
                    <div
                        key={idx}
                        className={`group relative p-6 rounded-2xl border border-border bg-card/50 backdrop-blur-sm hover:bg-card transition-all duration-300 hover:scale-[1.02] hover:shadow-xl ${item.border} flex flex-col justify-between h-full`}
                    >
